...
function initDeviceImage() {
  $("#device").empty();
  var str = '';
  str +=  '<img src="imgshow/device_outline.png" class="device-outline" style="z-index:1;"/>\
...
function initFunc() {
  ...
  if ($("#device").find(".show-F9").length <= 0) {
    ...
    $("#device").append(str);
    $("#device").find(".show-F9").css({"top": "-30px", "left": "285px", "display": "flex"});
  } else {
...
function initFuncEvent() { 
  ...
  $("#device").find(".show-F9").find("li").unbind('mouseenter mouseleave').hover(function() {
    ...
    var left = $(this).parent().position().left + $(this).position().left + $(this).width() / 2 - 75;
